  ...
  return (
    <div
      style={{
        ...
        background: "var(--gradient-dark-card-sweep)",
    ...
    >
        ...
        {TABS.map((t, i) => (
          <button
            ...
            style={{
              ...
              color: i === active ? "#fff" : "rgba(240,244,255,.8)",
              transition: "color .25s, border-color .25s",
            }}
          >
            <span style={{ font: "600 11px var(--font-mono)", letterSpacing: ".08em", color: i === active ? "var(--soft-violet)" : "rgba(240,244,255,.8)" }}>{t.num}</span>
      ...
      <div style={{ padding: "28px 26px" }}>
        <div className="ch-mcgrid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: "rgba(255,255,255,.07)", border: "1px solid rgba(255,255,255,.07)" }}>
          <div style={{ background: "var(--gradient-dark-card-violet)", padding: "24px 22px", boxShadow: "var(--edge-highlight-top, inset 0 1px 0 rgba(255,255,255,.06))" }}>
            ...
            <p style={{ margin: 0, font: "400 13.5px/1.55 var(--font-primary)", color: "rgba(255,255,255,.8)" }}>{tab.copy}</p>
          </div>
          <div style={{ background: "var(--gradient-dark-card-slate)", padding: "24px 22px", boxShadow: "var(--edge-highlight-top, inset 0 1px 0 rgba(255,255,255,.06))" }}>
            <span style={{ font: "600 11px var(--font-mono)", letterSpacing: ".12em", textTransform: "uppercase", color: "rgba(255,255,255,.8)" }}>Trazabilidad</span>
            <h3 style={{ margin: "11px 0 9px", font: "600 18px var(--font-primary)", letterSpacing: "-.02em", color: "#fff" }}>Señal → Decisión → Proyecto</h3>
            <p style={{ margin: 0, font: "400 13.5px/1.55 var(--font-primary)", color: "rgba(255,255,255,.8)" }}>Lo importante no se pierde entre juntas, documentos y chats.</p>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: "rgba(255,255,255,.07)", border: "1px solid rgba(255,255,255,.07)", borderTop: 0 }}>
          {STATS.map((s) => (
            <div key={s.label} style={{ background: "var(--surface-dark-tertiary)", padding: "18px 20px" }}>
              ...
              <div style={{ marginTop: 7, font: "600 11px var(--font-mono)", letterSpacing: ".08em", textTransform: "uppercase", color: "rgba(255,255,255,.8)" }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
