  ...
  return (
    <div ref={ref} style={{ maxWidth: 1020, margin: "0 auto", background: "linear-gradient(155deg,rgba(255,255,255,.94),rgba(244,242,250,.62))", border: "1px solid var(--border-subtle)", boxShadow: "0 30px 90px rgba(31,17,72,.08)" }}>
      ...
      <div style={{ position: "relative", padding: "52px 36px 40px" }}>
        ...
        <div className="mf-line" style={{ position: "absolute", left: `${INSET}%`, right: `${INSET}%`, top: 78, height: 2, background: "var(--line-structural)", opacity: 0.45, transformOrigin: "left center", transform: inView ? "scaleX(1)" : "scaleX(0)", transition: "transform .9s var(--ease-premium)" }} />
        <div className="mf-line" style={{ position: "absolute", left: `${INSET}%`, top: 78, height: 2, width: `${fill}%`, background: "linear-gradient(90deg,var(--signal-cyan),var(--change-violet))", transformOrigin: "left center", transform: inView ? "scaleX(1)" : "scaleX(0)", transition: "width .45s var(--ease-premium), transform .9s var(--ease-premium)" }} />
        ...
        <div className="mf-nodes" style={{ position: "relative", display: "grid", gridTemplateColumns: `repeat(${NODES.length},1fr)`, gap: 8 }}>
          {NODES.map((n, i) => {
            ...
            return (
              ...
              >
                <span aria-hidden="true" style={{ display: "block", marginBottom: 14, font: "300 clamp(26px,2.8vw,38px)/1 var(--font-secondary)", color: on ? "var(--change-violet)" : "var(--strategic-gray)", transition: "color .3s" }}>
                ...
                />
                <strong style={{ display: "block", font: "600 clamp(14px,1.3vw,16px) var(--font-primary)", letterSpacing: "-.01em", color: "var(--ink-graphite)", opacity: on ? 1 : 0.62, transition: "opacity .3s" }}>{n.title}</strong>
        ...
        {/* panel de detalle */}
        <div style={{ margin: "36px 0 0", padding: "30px 32px", border: "1px solid var(--border-subtle)", borderLeft: `3px solid ${node.color}`, background: "#fff", transition: "border-color .35s var(--ease-premium)" }}>
