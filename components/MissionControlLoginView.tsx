...
const LG = {
  es: {
    badge: "Mission Control · Memoria Viva",
    h1: "Acceso exclusivo a Mission Control",
    p: "Mission Control sostiene el porqué de cada decisión que el board acompaña. Si tu organización es cliente de Change, tu acceso fue entregado en la primera sesión de trabajo.",
    sec: ["Conexión cifrada extremo a extremo.", "Autenticación de dos factores en cada inicio de sesión.", "Sesiones expiran tras 20 minutos de inactividad."],
    signin: "Iniciar sesión", welcome: "Bienvenida al sistema.",
    userL: "Usuario o correo", userP: "nombre@organizacion.com", pwdL: "Contraseña",
    remember: "Mantener sesión por 8 horas", forgot: "¿Olvidaste tu acceso?",
    validating: "Validando…", enter: "Entrar al sistema",
    ssoSep: "O usa SSO corporativo", ssoBtn: "Continuar con SSO",
    restricted: "Acceso restringido", infoPre: "El sistema está en transición a SSO empresarial. Para tu acceso temporal, escríbenos desde la ", contactLink: "página de contacto", infoPost: ".",
    ver: "v1.4 · Memoria Viva", backMC: "← Volver a Mission Control",
    footer: "Cualquier intento de acceso no autorizado queda registrado · Change Intelligence",
  },
  en: {
    badge: "Mission Control · Living Memory",
    h1: "Exclusive access to Mission Control",
    p: "Mission Control holds the why behind every decision the board supports. If your organization is a Change client, your access was delivered in the first working session.",
    sec: ["End-to-end encrypted connection.", "Two-factor authentication on every sign-in.", "Sessions expire after 20 minutes of inactivity."],
    signin: "Sign in", welcome: "Welcome to the system.",
    userL: "Username or email", userP: "name@organization.com", pwdL: "Password",
    remember: "Keep session for 8 hours", forgot: "Forgot your access?",
    validating: "Validating…", enter: "Enter the system",
    ssoSep: "Or use corporate SSO", ssoBtn: "Continue with SSO",
    restricted: "Restricted access", infoPre: "The system is transitioning to enterprise SSO. For temporary access, write to us from the ", contactLink: "contact page", infoPost: ".",
    ver: "v1.4 · Living Memory", backMC: "← Back to Mission Control",
    footer: "Any unauthorized access attempt is logged · Change Intelligence",
  },
};
...
export function LoginView({ lang }: { lang: Lang }) {
  const t = LG[lang];
  ...
  return (
    <div className="change-dark" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "var(--surface-dark)" }}>
      <Header variant="dark" showCta={false} lang={lang} />
      <main
        id="main-content"
        style={{
          flex: 1,
      ...
      >
        ...
        >
          {/* ── Branding column ── */}
          <div>
            <Link href={localizeHref("/", lang)} aria-label="Change" style={{ display: "block", width: "fit-content", marginBottom: 30 }}>
              <Image
            ...
            <ul style={{ margin: "32px 0 0", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 12 }}>
              {t.sec.map((s) => (
                <li key={s} style={{ display: "flex", gap: 11, alignItems: "flex-start", font: "400 13.5px/1.5 var(--font-primary)", color: "rgba(240,244,255,.62)" }}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <FieldRow>
                <label htmlFor="mcl-user" style={FIELD_LABEL}>{t.userL}</label>
                <input
                  id="mcl-user"
                  name="user"
                  type="text"
              ...
              <FieldRow>
                <label htmlFor="mcl-pwd" style={FIELD_LABEL}>{t.pwdL}</label>
                <input
                  id="mcl-pwd"
                  name="password"
                  type="password"
              ...
              >
                {status === "submitting" ? t.validating : t.enter}
              </button>
